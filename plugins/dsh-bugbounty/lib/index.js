	...
	},
{
	...
	},
	{
		...
		techniques: ["RFC822 edge-case battery", "OAST domain as email domain", "CRLF header injection in emails", "unicode homograph emails", "differential user enumeration", "transactional-email HTML sink audit", "inert-param to later-triggered email chain", "Message-ID reuse bypass", "SMTP command-level injection & REFERER-header email poisoning (RCPT TO breakout, RFC-address bypass, SMTP-reply error-JSON oracle)"]
	},
	{
	...
	},
{
	...
	},
	{
		...
		techniques: ["bb_cache_key_probe", "Age/X-Cache analysis", "?cb= safe MISS test", "WCD static suffix", "X-Forwarded-Host unkeyed", "Akamai hop-by-hop", "unkeyed non-host cache-key battery (credential-scheme/Cookie/Accept-Encoding/X-HTTP-Method-Override, smuggling-delivered poisoning, poisoning-as-persistent-DoS)"]
	},
	{
		slug: "llm-ai",
