...
const CHECKLIST = [
	...
	},
	{
		...
		techniques: ["bb_wayback_urls (find id params)", "burp auth analyzer", "role swap", "method override", "Base64 ID swap", "Burp Intruder enumeration", "derivative-endpoint (scan/preview/export)", "live-ID polling", "auth-inheritance subdir probes", "verbosity-differential BFLA oracle", "revocation-gap re-test"]
	},
	{
		slug: "ssrf",
		name: "SSRF",
	...
	},
	{
		...
		techniques: ["bb_security_headers", "jwt_tool", "hashcat", "host header injection", "EditThisCookie", "session lifecycle tests", "serialized/signed cookie flip (flask-unsign, django-session-forger)", "re-auth matrix UI-vs-API differential", "derivative-path authz asymmetry (sort/filter/order)"]
	},
	{
		slug: "xss",
		name: "XSS",
	...
	},
	{
		...
		techniques: ["bb_enum_subdomains", "dig CNAME", "nuclei takeover templates", "can-i-take-over-xyz", "subzy run --concurrency 100 --hide_fails --verify_ssl", "GitHub namespace claim", "Shopify deleted-shop fingerprint", "Feed.Press podcast feed takeover", "post-claim cert/HTTPS escalation", "Azure Traffic Manager claim", "non-DNS link takeover", "web3 cookie-bombing + wallet-signing STO battery", "ESP/email-claim (Mailgun-class) takeover", "registrar-purchase of lapsed apex", "GitLab Pages/Webflow/marketplace custom-domain claim"]
	},
	{
		slug: "reporting",
	...
	},
	{
		...
		techniques: ["SameSite=lax bypass", "CORS audit", "OAuth redirect chain", "bb_wayback_urls (find redirect params)", "signature param method-flip CSRF", "destructive action re-auth check", "DNS-rebinding CSRF on device UIs", "CSRF-token hygiene battery", "_method form-override + Origin-less browsers", "login/logout-CSRF battery"]
	},
	{
		slug: "file-upload",
		...
		techniques: ["polyglot files", "magic byte spoofing", "ImageMagick/XML payloads", "zipslip", "engine MIME render matrix", "mimeType param flip", "filename-metadata second-order XSS sink", "filename/Content-Disposition smuggling battery", "upload-pipeline parser/SSRF/DoS battery"]
	},
	{
		slug: "engagement",
	...
	},
{
	...
	},
	{
		...
		techniques: ["%0d%0a / %0a / %00%0d%0a variants", "GBK-encoded CR/LF %E5%98%8D%E5%98%8A", "nuclei cRlf.yaml", "loxs mass-CRLF scanner", "log-injection control bytes"]
	},
	{
		slug: "host-header",
	...
	},
	{
		...
		techniques: ["X-Forwarded-For/True-Client-IP/CF-Connecting-IP rotation", "mirror endpoint enumeration", "method + param-name switching", "proxychains / IP rotation", "OTP/2FA endpoint targeting", "per-recipient bulk-email semantics", "identifier-normalization bypass", "per-account limit keying & counter identity"]
	},
	{
		slug: "403-bypass",
	...
	},
	{
		...
		techniques: ["RFC822 edge-case battery", "OAST domain as email domain", "CRLF header injection in emails", "unicode homograph emails", "differential user enumeration", "transactional-email HTML sink audit", "inert-param to later-triggered email chain", "Message-ID reuse bypass"]
	},
	{
	...
	},
	{
		...
		techniques: ["x-middleware-subrequest + x-middleware-rewrite probes", "coffinxp nuclei-templates (nextjs-middleware-cache.yaml)", "Grafana icon_hash 2123863676 / title dorks", "CVE-2025-29927 / CVE-2025-4123 nuclei templates", "Atlassian OGNL endpoint matrix", "Oracle Forms WebUtil battery", "nginx Range overflow CVE-2017-7529", "firmware appliance CVE cross-ref", "Cisco ASA +CSCOU+/../+CSCOE+ 0296 battery", "jQuery<1.12 XHR auto-exec", "patch-diff variant hunting", "Tomcat default-install battery", "Windows LPE chain (cfg.xml ACL -> PrintSpoofer)", "framework/product CVE shelf battery (Rails/Jira/Struts/WSO2/Rocket.Chat)", "server-infra & dependency CVE battery", "Apache Solr admin-surface battery", "product-specific pre-auth RCE/file-read/helper-argv recipes", "appliance / SSL-VPN CVE shelf (Ivanti, ASA 3452, Array, GitLab 7028)"]
	},
	{
		...
		techniques: ["patch-differential retest ('fixed' = hypothesis)", "sibling-path/per-enum fix regression", "deployed-artifact verification", "mitigation-review + fix-PR mining", "patch-differential retest ('fixed' = hypothesis)", "sibling-path/per-enum fix regression", "deployed-artifact verification", "mitigation-review + fix-PR mining"],
	},
	{
		slug: "windows-lpe",
		...
		techniques: ["SeImpersonate -> PrintSpoofer/Potato SYSTEM", "unquoted service path + weak ACL swap", "path-following delete / directory-junction", "config ACL -> DB creds -> service-account abuse", "privilege-file ladder + openssl.cnf engine hijack", "SeImpersonate -> PrintSpoofer/Potato SYSTEM", "unquoted service path + weak ACL swap", "path-following delete / directory-junction", "config ACL -> DB creds -> service-account abuse", "privilege-file ladder + openssl.cnf engine hijack"],
	},
	{
		slug: "github-recon",
	...
	},
	{
		...
		techniques: ["gau|gf lfi|uro pipeline", "qsreplace FUZZ", "ffuf -mr root: regex match", "ffuf -request raw", "php://filter wrapper", "double-encoded traversal", "post-filter self-nesting bypass", "ASP.NET Control.ResolveUrl path sink", "traversal-driven arbitrary write", "access-log/session/environ poisoning LFI->RCE"]
	},
	{
		slug: "cors",
	...
	},
{
		...
		techniques: ["{{7*7}} engine-ID", "{{7*'7'}} type-confusion", "Jinja2 os.popen chain", "Twig exec filter", "FreeMarker CVE-2022-22954", "OGNL %{7*7} + \\u0027 bypass", "WebWork valueStack chain", "template-engine exposed-object walk", "rogue-jndi LDAP/RMI chain", "EL sandbox-bypass battery", "template file-include authz bypass"]
	},
	{
		slug: "xxe-injection",
	...
	},
	{
		...
		techniques: ["CL.TE probe", "TE.CL probe", "TE.TE obfuscation", "H2 downgrade H2.CL/TE", "response-queue poison", "smuggler.py detect", "open-param request capture", "Transfer-Encoding space-in-name evasion", "Git smart-protocol path smuggling", "front-end parser-evasion (TE header-name-space)"]
	},
	{
		slug: "race-condition",
		...
		techniques: ["identical-copies race", "partial-construction race", "Turbo Intruder single-packet", "HTTP/2 h2 race", "coupon/OTP double-redeem", "register+blank-token", "async multi-hop TOCTOU", "governance/state-machine parallel-flow races"]
	},
	{
	...
	},
	{
		...
		techniques: ["redirect_uri differential", "state/PKCE CSRF", "token re-exchange", "SAML XSW injection", "SAML signature strip", "legacy-login matrix", "Cognito/user-pool IdP-client abuse battery", "multi-IdP trust-boundary (collision, RelayState, SLO)"]
	},
	{
		slug: "idp-confusion",
	...
	},
	{
		...
		techniques: ["APKPure mirror pull", "jadx secret grep", "iOS OTA manifest.plist", "TestFlight beta diff", "API surface re-hunt", "Frida pinning bypass", "WebView URL spoofing", "WebView -> URI-scheme handler invocation", "ContentProvider save-flow filename traversal", "logcat secret leakage", "adb am start intent driving"]
	},
	{
	...
	},
	{
		...
		techniques: ["kubelet 10250 /pods", "API 6443 anonymous", "etcd 2379 dump", "docker.sock exec", "ServiceAccount RBAC check", "internal registry pull", "hostPath writable escape", "CVE-2020-28914 Kata/gVisor"]
	},
	{
	...
	},
	{
		...
		techniques: ["pull_request_target injection", "self-hosted runner RCE", "Jenkins 23897", "npm/PyPI confusion", "registry search", "pipeline secret hunt", "npm/PyPI local audit battery", "install-flow integrity & registry poisoning", "runner/artifact injection battery"]
	},
	{
...

	{
		...
		techniques: ["honggfuzz", "AFL persistent mode", "libFuzzer", "Atheris", "ASAN/UBSAN triage", "afl-tmin minimize", "CVE filing", "differential testing vs reference", "property-based invariant fuzz", "stateful REST API fuzzing (MOREST/RESTler)", "in-process ReDoS timing harness", "crypto correctness-differential output oracle"]
	},
